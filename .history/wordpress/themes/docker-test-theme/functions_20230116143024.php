...
function sailthru_first_paragraph_into_content( $content = '' ) {
    if( $content !== '' ) {
        ...
        $content = preg_replace( '#<table[^>]+>.*?</table>#is', '', $content ); #remove table
        $content = preg_replace( '#<a class=\W+aa_button[^>]+>.*?<\/a>#is', '', $content ); #remove buttons
        $content = preg_replace( '#<h3 class=\W+faq_que[^>]+>.*?<\/h3>#is', '', $content ); #remove FAQ heading
